...
COMMON_FLAGS = {
    ...
    "flip_intensity":   [False], # [False, True]
    "match_histogram":  [False, True],
    "clahe":            [False], # [False,  True]
...
def run_inner_cv_group(identity_names, configs, all_pairs, meta_df, outer_train_idx, seed=42):
    """
    ...
    for cfg_idx, config in enumerate(tqdm(configs, desc=f"  configs", leave=False)):
...
# ---------------------------------------------------------------------------
# Inner CV: перебор конфигов для одной metric identity (оставлен для обратной совместимости)
# ---------------------------------------------------------------------------

def run_inner_cv(identity_name, configs, all_pairs, meta_df, outer_train_idx, seed=42):
    """
    Для одной metric identity и одного outer fold:
      - перебирает все конфиги
      - для каждого: 4-fold inner CV на outer_train, усреднённые метрики по фолдам

    Возвращает: список dicts [{cfg_idx, config, inner_sp_mean, inner_auc_bad_mean, ...}, ...]
    """
    train_meta = meta_df.iloc[outer_train_idx].copy()
    train_pairs = [all_pairs[i] for i in outer_train_idx]
    ...
    results = []
    for cfg_idx, config in enumerate(tqdm(configs, desc=f"  {identity_name} configs", leave=False)):
        # один проход вычисления scores на всём outer_train
        scores_all = compute_scores_for_config(config, train_pairs, identity_name)

        fold_sp, fold_auc_bad = [], []
        for inner_tr, inner_va in inner_splits:
            # на каждом inner_val фолде
            inner_scores = scores_all[inner_va]
            inner_cls3 = y_class3[inner_va]
            inner_1_5 = y_1_5[inner_va]
...
# ---------------------------------------------------------------------------
# Outer CV: основной цикл
# ---------------------------------------------------------------------------

def run_outer_cv(dataset, identities=None, outer_seed=155):
    """
    ...
    Возвращает:
      oof_predictions: list[dict] — для каждой пары и каждой метрики:
          {sample_idx, metric, fold, similarity_score, config}
      fold_meta: list[dict] — для каждого outer fold:
          {fold, metric, config, inner_sp_mean, ...}
      meta_df: pd.DataFrame — метаданные (pname, class3, Similarity_Score) для evaluate_oof / bootstrap_oof
    """
    ...
    # кэшируем все пары и метаданные
    ...
    for fold_id, (outer_tr, outer_va) in enumerate(outer_splits):
        ...
        print(f"  outer_train: {len(outer_tr)} pairs, {len(np.unique(groups[outer_tr]))} WSI")
        print(f"  outer_val:   {len(outer_va)} pairs, {len(np.unique(groups[outer_va]))} WSI")
        ...
        for id_group in metric_groups:
            ...
            # убираем metric-specific поле из конфига (будет добавлено позже)
            for cfg in configs:
                cfg.pop("metric", None)
            ...
            group_results = run_inner_cv_group(
                id_group, configs, all_pairs, meta_df, outer_tr, seed=inner_seed
            )
